...
ws.on('open', () => {
  ...
  // Activer l'onglet formalibre.org (id 88)
  console.log('🌐 Activation onglet formalibre.org [88]...');
...
ws.on('message', (data) => {
  ...
  if (msg.commandId === 'activate') {
    console.log('✅ formalibre.org activé\n');
    setTimeout(() => {
  ...
  if (msg.commandId === 'shot') {
    if (msg.payload.dataUrl) {
      ...
      if (match) {
        fs.writeFileSync('formalibre-portfolio-click.png', Buffer.from(match[2], 'base64'));
